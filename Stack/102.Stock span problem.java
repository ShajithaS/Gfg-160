...
//Time Complexity:O(n)
/*The for loop runs O(n) times since it iterates through each element of the input array once.
Inside the loop, the while loop ensures that each element is pushed and popped from the stack at most once.
Since each element is processed in constant time and the stack operations (push and pop) are O(1) each, the worst-case scenario results in O(n) + O(n) = O(n).*/
//Space complexity:O(n) (due to the ArrayList and Stack)
